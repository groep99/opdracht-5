...
if [[ "$(id -u)" -ne 0 ]]; then
  err "Dit script moet als root draaien. Gebruik: curl ... | sudo bash"
fi
...
# no more neofetch -> fastfetch, fastfetch not available for ubuntu <=22.04
FETCH_PKG="fastfetch"
...
if ! id -nG "${REAL_USER}" | grep -qw docker; then
  ...
  info "Gebruiker '${REAL_USER}' toegevoegd aan de docker-groep (logoff/logon)."
fi
...
echo ""
echo " Vergeet niet:"
echo "   - Logoff/logon of 'newgrp docker' voor docker zonder sudo"
